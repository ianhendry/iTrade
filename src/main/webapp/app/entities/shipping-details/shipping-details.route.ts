...
export class ShippingDetailsResolve implements Resolve<IShippingDetails> {
  constructor(private service: ShippingDetailsService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IShippingDetails> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((shippingDetails: HttpResponse<ShippingDetails>) => {
...
export const shippingDetailsRoute: Routes = [
  {
    ...
    data: {
      ...
      defaultSort: 'id,asc',
      pageTitle: 'iTradeApp.shippingDetails.home.title',
    },
